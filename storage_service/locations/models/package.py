...
class Package(models.Model):
    ...
    def start_reingest(self, pipeline, reingest_type):
        ...
        if self.is_compressed:
            ...
            logging.debug('Reingest: extracted to %s', local_path)
        else:
            local_path = self.fetch_local_path()
            temp_dir = ''
            logging.debug('Reingest: uncompressed at %s', local_path)

        # Run fixity
        # Fixity will fetch & extract package if needed
        success, _, error_msg = self.check_fixity(delete_after=False)
        logging.debug('Reingest: Fixity response: %s, %s', success, error_msg)
        if not success:
        ...
        logging.info('Reingest: files: %s', reingest_files)
        ...
        logging.debug('Reingest: Current location: %s', current_location)
        ...
        reingest_url = 'http://' + pipeline.remote_name + '/api/sip/reingest'
        params = {
        ...
        logging.debug('Reingest: URL: %s; params: %s', reingest_url, params)
        try:
            response = requests.post(reingest_url, data=params, allow_redirects=True)
        except requests.exceptions.RequestException:
            logging.exception('Unable to connect to pipeline %s', pipeline)
    ...
    def finish_reingest(self, origin_location, origin_path, reingest_location, reingest_path):
        ...
        if self.origin_pipeline.uuid != self.misc_attributes.get('reingest_pipeline'):
            logging.info('Reingest: Received pipeline %s did not match expected pipeline %s', self.origin_pipeline.uuid, self.misc_attributes.get('reingest_pipeline'))
        ...
        if os.path.isfile(reingest_full_path):
            ...
            logging.info('Extracting with: {}'.format(command))
            rc = subprocess.call(command)
            logging.debug('Extract file RC: %s', rc)
            # Get output path
        ...
        if puid == 'fmt/484':  # 7 Zip
            ...
            if algo == 'bzip2':
                ...
                compression = self.COMPRESSION_7Z_BZIP
                logging.warning('Reingest: Unable to determine reingested compression algorithm, defaulting to bzip2.')
        elif puid == 'x-fmt/268':  # Bzipped (probably tar)
            compression = self.COMPRESSION_TAR_BZIP2
        else:
            compression = self.COMPRESSION_7Z_BZIP
            logging.warning('Reingest: Unable to determine reingested file format, defaulting recompression algorithm to %s.', compression)
        logging.info('Reingest: compressing with %s', compression)
        ...
        if compression in (self.COMPRESSION_7Z_BZIP, self.COMPRESSION_7Z_LZMA):
            ...
            logging.warning('Unknown compression algorithm, cannot correctly update pointer file')
